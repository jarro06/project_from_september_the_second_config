...
def job_config = [
    job: [
        name: "project_from_september_the_second_Pipeline_develop"
    ],
    git: [ 
        branch: "develop"
    ]
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "project_from_september_the_second_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
            steps {
                echo "Build"
                script {
                    dir ("project_from_september_the_second") {
                        jslMaven.build()
                    }
                }
            }    
